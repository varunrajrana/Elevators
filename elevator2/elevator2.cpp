...
int main()
{
	// Completion semaphores to make elevator go to ground and open, then exit
	...
	CCondition *EV2DW[10];
	for (int i = 0; i < 10; ++i) {
		EV2UP[i] = new CCondition(to_string(i) + "ev2up", AUTORESET);
		EV2DW[i] = new CCondition(to_string(i) + "ev2dw", AUTORESET);
	}
				...
				// If our elevator doors are closed and the current floor doors are closed we can open them.
						...
						if (dest_queue.size() > 0){
							...
							if (update.direction == UP){
								EV2UP[(int)update.curr_floor]->Signal();
							}
							else {
								EV2DW[(int)update.curr_floor]->Signal();
							}
						}
